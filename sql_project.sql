...
SELECT DATE_FORMAT(Date, '%Y-%m') AS Month,ROUND(SUM(TotalAmount), 2) AS Total_Monthly_Payroll
FROM Payroll GROUP BY date_format(Date, '%Y-%m'), MONTH(Date) ORDER BY Month ASC;
